...
<?php endforeach; ?>
    ...
    <script>
        ...
        // Function to show custom alert with GIF and message
function showCustomAlert(message) {
    document.getElementById('customAlertMessage').textContent = message; // Set the custom message
    var alertModal = new bootstrap.Modal(document.getElementById('customAlertModal'));
    alertModal.show(); // Show the modal
}

// Replace the default alert with showCustomAlert in existing scripts
window.onload = function() {
    ...
    document.addEventListener('visibilitychange', function() {
        if (document.hidden) {
            tabSwitchCount++;
            showCustomAlert('Are you trying to earn a PhD in malpractices? Keep it up, and you might just graduate with honors!.Switching tabs is not allowed!'); // Use custom alert instead of default alert
            if (tabSwitchCount >= allowedTabSwitches) {
                showCustomAlert(' Time up! You have been logged out for malpractice. Please return when you are ready to take the test honestly!');
                setTimeout(function() {
                    document.getElementById('test').submit(); // Automatically submit the test
                }, 2000); // Wait for 2 seconds before submitting to show alert
            }
        }
    });
    ...
    document.addEventListener('keydown', function(e) {
        if ((e.altKey && e.key === 'Tab') || (e.key === 'F12') || 
            ...
            e.preventDefault();
            showCustomAlert('Are you trying to earn a PhD in malpractices? Keep it up, and you might just graduate with honors!');
        }
    });
};
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <img src="../images/alert.gif" alt="Alert GIF" class="img-fluid mb-3" style="width: 150px;">
                <p id="customAlertMessage"></p> <!-- Dynamic alert message -->
            </div>
